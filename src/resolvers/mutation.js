...
const Mutation = {
    ...
    removerPessoa (parent, args, ctx, info){
        ...
        livros = ctx.db.livros.filter (livro => {
            const remover = livro.autor === args.id
            comentarios = ctx.db.comentarios.filter(c => remover && c.livro !== livro.id)
            return !remover
        })
        comentarios = ctx.db.comentarios.filter(c => c.autor !== args.id)
        return removido
    ...
    removerLivro(parent, args, ctx, info){
        const indice = ctx.db.livros.findIndex(
            livro => livro.id === args.id
        )
        if (indice < 0){
            throw new Error ("Livro não existe");
        }
        const removido = ctx.db.livros.splice(indice, 1)[0]
        ctx.db.comentarios = ctx.db.comentarios.filter(
            c => c.livro  !== args.id
        );
        ctx.pubSub.publish('livro', {livro: {
            mutation: 'remocao',
    ...
    removerComentario(parent, args, ctx, info){
        const indice = ctx.db.comentarios.findIndex(
            c => c.id === args.id
        )
        if (indice < 0){
    ...
    atualizarComentario (parent, {id,comentario}, {db}, info){
        ...
        if(!comentarioExistente)
            throw new Error ("Comentário não existente")
        Object.assign(
            comentarioExistente,{
